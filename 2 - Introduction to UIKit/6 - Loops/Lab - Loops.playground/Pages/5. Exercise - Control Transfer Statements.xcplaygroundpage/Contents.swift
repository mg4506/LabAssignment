...
for (index,letter) in alphabet.enumerated(){
    if(index%2==0){
        print("even character at even index number is \(letter) at index \(index)")
    }
}
...
var movementHeartRates: [String: String] = ["Delhi":"New Delhi","Mumbai":"Maharastra","Goa":"Panaji"]
    for (state,capital) in movementHeartRates{
        if(state=="Delhi"){
